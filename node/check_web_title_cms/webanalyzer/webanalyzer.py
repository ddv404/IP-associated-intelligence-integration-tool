...
class WebAnalyzer(object):
    ...
    def _request(self, url: str,my_header, my_body, my_title) -> hash:
 

        script = []
        meta = {}

        p = BeautifulSoup(my_body, "html5lib")
        ...
        raw_headers = '\n'.join('{}: {}'.format(k, v) for k, v in my_header.items())
        self._targets[url] = {
            "url": url,
            "body": my_body,
            "headers": my_header,
            ...
            "meta": meta,
            "title": title,
            "cookies": my_header["cookies"] if "cookies" in my_header else "",
            "raw_cookies": my_header["raw_cookies"] if "raw_cookies" in my_header else "",
            "raw_response": raw_headers + my_body,
            "raw_headers": raw_headers,
            "md5": hashlib.md5(my_body.encode("utf8")).hexdigest(),
        }
    ...
    def start(self, url: str,my_header, my_body, my_title, reload: bool = True):
        ...
        if not self._request(url,my_header, my_body, my_title):
...
def check(url, update,my_header, my_body, my_title):
    w = WebAnalyzer()
    ...
    r = w.start(url,my_header, my_body, my_title)
    # if r:
